...
public class TranslateVisitor implements codegen.C.Visitor {
	...
	public void visit(codegen.C.exp.And e) {
		...
		e.right.accept(this);
		cfg.operand.T right = this.operand;
		emit(new cfg.stm.And(dst, new cfg.type.Int(), left, right));
	...
	public void visit(codegen.C.exp.ArraySelect e) {
		...
		emit(new cfg.stm.ArraySelect(dst, new cfg.type.Int(), array, index));
		this.operand = new cfg.operand.Var(dst);
	}
	...
	public void visit(codegen.C.exp.Length e) {
		String dst = this.genVar();
		e.array.accept(this);
		cfg.operand.T array = this.operand;
		emit(new cfg.stm.Length(dst, new cfg.type.Int(), array));
		this.operand = new cfg.operand.Var(dst);
	}
	...
	public void visit(codegen.C.exp.Lt e) {
		...
		cfg.operand.T left = this.operand;
		e.right.accept(this);
		emit(new cfg.stm.Lt(dst, new cfg.type.Int(), left, this.operand));
		this.operand = new cfg.operand.Var(dst);
		return;
	}
	...
	public void visit(codegen.C.exp.NewIntArray e) {
		...
		emit(new cfg.stm.NewIntArray(dst, new cfg.type.IntArray(), exp));
		this.operand = new cfg.operand.Var(dst);
	}
	...
	public void visit(codegen.C.exp.Not e) {
		...
		e.exp.accept(this);
		cfg.operand.T exp = this.operand;
		emit(new cfg.stm.Not(dst, new cfg.type.Int(), exp));
		this.operand = new cfg.operand.Var(dst);
	}
	...
	public void visit(codegen.C.stm.Assign s) {
		s.exp.accept(this);
		emit(new cfg.stm.Move(s.id, null, this.operand, s.idIsField));
		return;
	}
	...
	public void visit(codegen.C.stm.AssignArray s) {
		...
		s.index.accept(this);
		cfg.operand.T index = this.operand;
		emit(new cfg.stm.MoveArray(s.id, null, index, exp, s.idIsField));
	}
	...
	public void visit(codegen.C.stm.Block s) {
		...
		emit(new cfg.stm.Block(this.stmOrTransfer));
	}
	...
	public void visit(codegen.C.method.Method m) {
		...
		// separate the stmOrTransfer into blocks;
		// up to now,only if and while generate new label;
